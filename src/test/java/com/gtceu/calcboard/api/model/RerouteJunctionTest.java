...
public class RerouteJunctionTest {
    ...
    @Test
    public void testUnconnectedSourceJunctionSuppliesUnlimitedDemand() {
        FlowGraph graph = new FlowGraph();

        // Unconnected Source Junction (Raw Diamond provider)
        RecipeNode diamondJunction = RecipeNode.createReroute(100.0, 100.0);
        diamondJunction.bindRerouteIngredient(IngredientStack.item(ResourceLocation.tryParse("minecraft:diamond"), "Diamond", 1.0));
        graph.addNode(diamondJunction);

        // 4 downstream consumers demanding arbitrary amounts of diamond (e.g. Boilers)
        RecipeNode boiler1 = RecipeNode.create("Boiler 1", 20.0, 0.0, GTVoltageTier.LV);
        boiler1.getInputs().add(IngredientStack.item(ResourceLocation.tryParse("minecraft:diamond"), "Diamond", 1.0));
        boiler1.setMachineCount(1.0); // 1.0 diamond/s
        graph.addNode(boiler1);

        RecipeNode boiler2 = RecipeNode.create("Boiler 2", 20.0, 0.0, GTVoltageTier.LV);
        boiler2.getInputs().add(IngredientStack.item(ResourceLocation.tryParse("minecraft:diamond"), "Diamond", 1.0));
        boiler2.setMachineCount(4.0); // 4.0 diamond/s
        graph.addNode(boiler2);

        RecipeNode boiler3 = RecipeNode.create("Boiler 3", 20.0, 0.0, GTVoltageTier.LV);
        boiler3.getInputs().add(IngredientStack.item(ResourceLocation.tryParse("minecraft:diamond"), "Diamond", 1.0));
        boiler3.setMachineCount(16.0); // 16.0 diamond/s
        graph.addNode(boiler3);

        RecipeNode boiler4 = RecipeNode.create("Boiler 4", 20.0, 0.0, GTVoltageTier.LV);
        boiler4.getInputs().add(IngredientStack.item(ResourceLocation.tryParse("minecraft:diamond"), "Diamond", 1.0));
        boiler4.setMachineCount(65.0); // 65.0 diamond/s
        graph.addNode(boiler4);

        // Connect junction output (port 0) to all 4 boilers
        graph.addConnection(diamondJunction.getId(), 0, boiler1.getId(), 0);
        graph.addConnection(diamondJunction.getId(), 0, boiler2.getId(), 0);
        graph.addConnection(diamondJunction.getId(), 0, boiler3.getId(), 0);
        graph.addConnection(diamondJunction.getId(), 0, boiler4.getId(), 0);

        // Compute efficiencies: all boilers should operate at 100% without bottleneck/deficit
        var effMap = FlowBalanceMatrixSolver.computeNodeEfficiencies(graph);
        Assertions.assertEquals(1.0, effMap.get(boiler1.getId()), 0.001);
        ...
        // Verify port stats
        var inStats1 = graph.getInputPortStats(boiler1, 0);
        Assertions.assertEquals(1.0, inStats1.requiredOrProducedRate(), 0.001);
        Assertions.assertEquals(1.0, inStats1.connectedRate(), 0.001);
        Assertions.assertFalse(inStats1.isInputDeficit());

        var inStats4 = graph.getInputPortStats(boiler4, 0);
        Assertions.assertEquals(65.0, inStats4.requiredOrProducedRate(), 0.001);
        Assertions.assertEquals(65.0, inStats4.connectedRate(), 0.001);
        Assertions.assertFalse(inStats4.isInputDeficit());

        // Verify junction output port stats
        var outStats = graph.getOutputPortStats(diamondJunction, 0);
        double totalDemand = 1.0 + 4.0 + 16.0 + 65.0; // 86.0 diamond/s
        Assertions.assertEquals(totalDemand, outStats.requiredOrProducedRate(), 0.001);
        Assertions.assertEquals(totalDemand, outStats.connectedRate(), 0.001);

        // Verify summary aggregations
        BalanceSummary summary = graph.computeSummary();
        IngredientStack diamondKey = IngredientStack.item(ResourceLocation.tryParse("minecraft:diamond"), "Diamond", 1.0);
        Assertions.assertTrue(summary.rawInputs().containsKey(diamondKey));
        Assertions.assertEquals(totalDemand, summary.rawInputs().get(diamondKey), 0.001);
    }

    @Test
    public void testShiftConnectAndAutoRatioIntoJunctionAggregatesDownstreamDemands() {
        FlowGraph graph = new FlowGraph();
        ...
        // 1. Junction Node
        RecipeNode diamondJunction = RecipeNode.createReroute(100.0, 100.0);
        diamondJunction.bindRerouteIngredient(IngredientStack.item(diamondId, "Diamond", 1.0));
        graph.addNode(diamondJunction);

        // 2. 4 Boilers demanding 1.0, 4.0, 16.0, 65.0 Diamond/s (total 86.0 Diamond/s)
        RecipeNode boiler1 = RecipeNode.create("Boiler 1", 20.0, 0.0, GTVoltageTier.LV);
        boiler1.getInputs().add(IngredientStack.item(diamondId, "Diamond", 1.0));
        boiler1.setMachineCount(1.0);
        graph.addNode(boiler1);

        RecipeNode boiler2 = RecipeNode.create("Boiler 2", 20.0, 0.0, GTVoltageTier.LV);
        boiler2.getInputs().add(IngredientStack.item(diamondId, "Diamond", 1.0));
        boiler2.setMachineCount(4.0);
        graph.addNode(boiler2);

        RecipeNode boiler3 = RecipeNode.create("Boiler 3", 20.0, 0.0, GTVoltageTier.LV);
        boiler3.getInputs().add(IngredientStack.item(diamondId, "Diamond", 1.0));
        boiler3.setMachineCount(16.0);
        graph.addNode(boiler3);

        RecipeNode boiler4 = RecipeNode.create("Boiler 4", 20.0, 0.0, GTVoltageTier.LV);
        boiler4.getInputs().add(IngredientStack.item(diamondId, "Diamond", 1.0));
        boiler4.setMachineCount(65.0);
        graph.addNode(boiler4);
        ...
        // 3. Sieve producing 0.4 Diamond/s per machine (duration: 40s = 800 ticks, amount: 16)
        RecipeNode sieve = RecipeNode.create("Mechanical Sieve", 800.0, 30.0, GTVoltageTier.LV);
        sieve.getOutputs().add(IngredientStack.item(diamondId, "Diamond", 16.0));
        sieve.setMachineCount(1.0);
        graph.addNode(sieve);

        // Shift-connect sieve -> diamondJunction: calculateProducerMatchCount
        double matchedCount = FlowGraphSolver.calculateProducerMatchCount(graph, sieve, 0, diamondJunction, 0);
        // Total demand is 86.0 / (16.0 / 40.0 = 0.4/s) = 215 machines!
        Assertions.assertEquals(215.0, matchedCount, 0.001);

        sieve.setMachineCount(matchedCount);
        graph.addConnection(sieve.getId(), 0, diamondJunction.getId(), 0);

        // Verify all 4 boilers are 100% satisfied
        var effMap = FlowBalanceMatrixSolver.computeNodeEfficiencies(graph);
        Assertions.assertEquals(1.0, effMap.get(boiler1.getId()), 0.001);
        Assertions.assertEquals(1.0, effMap.get(boiler2.getId()), 0.001);
        Assertions.assertEquals(1.0, effMap.get(boiler3.getId()), 0.001);
