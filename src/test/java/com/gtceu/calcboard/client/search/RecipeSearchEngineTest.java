...
public class RecipeSearchEngineTest {
    ...
    public void testBracketedCategoryQueryParsing() {
        var query = RecipeSearchEngine.parseQuery("[gas turbine]");
        assertFalse(query.isEmpty());
        assertEquals(1, query.orGroups().size());
        var terms = query.orGroups().get(0).terms();
        assertEquals(1, terms.size());
        assertEquals("gas turbine", terms.get(0).text());
        assertEquals(RecipeSearchEngine.MatchType.CATEGORY, terms.get(0).type());
        assertFalse(terms.get(0).negated());

        var query2 = RecipeSearchEngine.parseQuery("[chemical reactor] nitrobenzene");
        assertFalse(query2.isEmpty());
        var terms2 = query2.orGroups().get(0).terms();
        assertEquals(2, terms2.size());
        assertEquals("chemical reactor", terms2.get(0).text());
        assertEquals(RecipeSearchEngine.MatchType.CATEGORY, terms2.get(0).type());
        assertEquals("nitrobenzene", terms2.get(1).text());
        assertEquals(RecipeSearchEngine.MatchType.GENERAL, terms2.get(1).type());
    }
}
