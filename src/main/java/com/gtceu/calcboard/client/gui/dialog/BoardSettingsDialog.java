...
public class BoardSettingsDialog implements IBoardModal {
    ...
    public void render(GuiGraphics graphics, int screenWidth, int screenHeight, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            if (tabs[i] == SettingsTab.UPDATES && ClientUpdateNotifier.getInstance().isBadgeVisible()) {
        ...
        switch (tabs[activeTab]) {
            ...
            case UPDATES -> renderUpdatesTab(graphics, font, mainX, mainY, mainW, mainH, mouseX, mouseY);
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        switch (tabs[activeTab]) {
            ...
            case UPDATES -> handleUpdatesClick(mouseX, mouseY, mainX, mainY, mainW);
        }
    ...
    private void renderUpdatesTab(GuiGraphics graphics, Font font, int x, int y, int w, int h, int mouseX, int mouseY) {
    ...
    private void handleUpdatesClick(double mouseX, double mouseY, int x, int y, int w) {
        ClientUpdateNotifier notifier = ClientUpdateNotifier.getInstance();
        int cardY = y + 16;
        int cw = w - 8;
    ...
    private void toggleConfigBoolean(net.minecraftforge.common.ForgeConfigSpec.BooleanValue configValue) {
    ...
    private void openDownloadUrl(String url) {
        ...
        mc.setScreen(new ConfirmLinkScreen(confirmed -> {
            if (confirmed) {
                net.minecraft.Util.getPlatform().openUri(URI.create(url));
            }
            mc.setScreen(parent);
        }, url, true));
    }
