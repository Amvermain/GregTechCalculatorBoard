...
## 6. 전역 종합 수지 대시보드 (Global Factory Balance Dashboard)

팀 워크스페이스 내 여러 공정 페이지에 걸친 **전 기지 통합 생산량/소비량/전력 총합 계산기**를 제공합니다.

### 6.1 기능 개요 & 필요성
* **기존 한계**: 개별 페이지 단위의 `SummaryOverlay`는 해당 페이지 내부의 입출력만 보여주므로, '페이지 1(에틸렌 생산)'과 '페이지 2(플라스틱 소비)'가 합쳐졌을 때 전체 기지에서 에틸렌이 남는지 부족한지 한눈에 파악하기 어렵습니다.
* **해결책 (`GlobalBalanceDashboardDialog`)**: 툴바의 `[📊 전역 수지]` 버튼을 누르면, **원하는 페이지들을 체크박스로 선택하여 합산된 전 기지 대차대조표**를 실시간으로 집계/분석합니다.

```mermaid
graph LR
    P1["[✔] Page 1: 석유 화학<br>(에틸렌 +500 mB/s, -2k EU/t)"] --> Engine["Global Balance Aggregator"]
    P2["[✔] Page 2: 플라스틱 사출<br>(에틸렌 -300 mB/s, -5k EU/t)"] --> Engine
    P3["[✔] Page 3: 벤젠 발전소<br>(전력 +30k EU/t)"] --> Engine
    P4["[ ] Page 4: 광물 처리<br>(비활성 / 제외)"] -.->|제외됨| Engine

    Engine --> Result["📊 전 기지 통합 수지 결과:<br>• 순 에틸렌: +200 mB/s (잉여 🟢)<br>• 순 전력: +23,000 EU/t (안전 🟢)"]
```

### 6.2 대시보드 UI 명세 (`GlobalBalanceDashboardDialog`)

```text
+---------------------------------------------------------------------------------------------------+
| 📊 Global Factory Balance Dashboard                                                           [X] |
+---------------------------------------------------------------------------------------------------+
| [ 좌측: 포함할 페이지 선택 ]               | [ 우측: 선택된 페이지 통합 대차대조표 ]                    |
| [✔] 1. 석유 정제 & 에틸렌 라인             | ⚡ 전 기지 총 전력: +23,000.0 EU/t (순 잉여 🟢)              |
...
### 6.3 주요 세부 기능
1. **필터링 & 페이지 조합 프리셋**:
   * "주요 석유화학 콤플렉스", "핵연료 주기 라인" 등 자주 비교하는 페이지 묶음을 프리셋으로 저장/로드.
2. **품목별 상세 기여도 분석 (Item Flow Breakdown)**:
   * 목록에서 `Liquid Oxygen`을 클릭하면:
     - `Page 1 (원유 공정)`: `+100 mB/s` (생산)
     - `Page 2 (플라스틱 공정)`: `-250 mB/s` (소비)
     - `→ 순 결손: -150 mB/s (원유 공정을 2.5배 증설하거나 전해조 추가 필요)` 가이드 제공.
...
+-----------------------------------------------------------------------------------------------------------------+
| [+ Page 1] [+ Page 2] | 📖 Guide  🎯 Ratio  🚀 Flow  📦 Group  [📊 전역 수지]  [🚀 팀 커밋]  [🔀 팀에 머지]  [📜 히스토리] |
+-----------------------------------------------------------------------------------------------------------------+
```

### 7.2 주요 신규 다이얼로그 UI
1. **`GlobalBalanceDashboardDialog`**: 다중 페이지 선택 기반 전 기지 통합 수지 대차대조표.
2. **`CommitDialog`**: 커밋 요약(추가/수정/삭제 노드 수) 확인 및 커밋 메시지 작성.
3. **`CommitHistoryDialog`**: 리비전 타임라인 탐색, 커밋별 Diff 요약, 미리보기 및 롤백.
4. **`VisualMergeResolverDialog`**: 3-Way 머지 시 노드 속성 충돌을 좌/우 비교하여 클릭으로 채택.

---

## 8. 개발 단계 및 마일스톤 (Phased Implementation)

```mermaid
gantt
    title v2.0.0 구현 로드맵 (Git 스타일 협업 & 전역 수지 대시보드)
    ...
    section Phase 3: 머지 엔진 & 전역 집계
    3-Way GraphMergeEngine (자동 머지/충돌)     :p3_1, after p2_2, 4d
    Global Balance Aggregator (다중 페이지 합산) :p3_2, after p3_1, 3d
    section Phase 4: UI & 클라이언트
    전역 수지 대시보드 & 히스토리 뷰어 GUI       :p4_1, after p3_2, 4d
...
본 RFC의 **Git 스타일 브랜치/포크, 3-Way 지능형 머지, 커밋 히스토리 타임머신, 그리고 전역 종합 수지 대시보드**는 마인크래프트 테크 모드 역사상 가장 진보된 협업 환경을 제공할 것입니다.
팀원들이 서로의 작업을 덮어쓸 걱정 없이 각자의 개인 보드에서 자유롭게 실험하고, 완성된 공정을 팀 보드에 안전하게 병합하며, 기지 전체의 생산/소비 밸런스를 한눈에 통제할 수 있는 완벽한 아키텍처를 완성합니다.
