...
* **상황별 5종 경고 뱃지 (`NodeBadgeRegistry`)**:
  - `[⚠️ Loop]`: 외부 보충 원료가 부족한 폐순환 루프에서 스케일링 억제.
  - `[⚠️ Growth]`: 사이클 당 유량이 증폭되는 양의 피드백 증식 루프.
  - `[⚠️ Catalyst]`: 미세 확률 감쇠가 발생하는 촉매 루프.
  - `[⚠️ Conflict]`: 상호 모순되는 복수의 기준 앵커가 지정된 경로.
  - `[⚠️ Yield]`: $10^{-5}$ 미만 극미세 수율 레시피.
* **5행 대화형 진단 툴팁**:
