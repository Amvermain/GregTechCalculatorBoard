...
### 3. 기준 레시피 지정 & 정수 올림 비율 맞춤 (Auto Ratio)
- 생산량을 맞추고 싶은 핵심 기계나 최종 완성품을 **기준(Base)**으로 지정할 수 있습니다.
- **비율 맞춤(Auto Ratio)** 버튼을 누르면 지정한 기준 기계의 생산/소비량에 맞춰 연결된 앞뒤 모든 기계 대수를 **정수($1, 2, 3...$)로 올림(`Ceiling`)**하여 맞춰줍니다.
- 상류 공급 기계의 생산 속도가 하류 소비 기계의 요구 속도보다 항상 크거나 같도록($\text{Supply} \ge \text{Demand}$) 보장하여 공정에서 원료 부족으로 인한 병목(Starvation)이 발생하지 않습니다.

### 4. 그렉텍 대형 터빈 / 발전기 연산 & 48종+ 로터 스캐너
- **발전기 전용 에메랄드 테마**: 터빈/발전기 카드는 딥 에메랄드 그린 패널(`+EU/t`)로 차별화되어 한눈에 구별됩니다.
- **3D 비주얼 로터 선택창 (`[⚙ 220%]` 클릭)**: 그렉텍 기본 로터 및 모드팩 커스텀 로터 48종 이상을 3D 아이템 그리드로 시각화하여, 원하는 로터를 클릭 한 번으로 장착할 수 있습니다.
- **로터 파워 & 티어 기반 정격 병렬 자동 도출**: 로터나 전압 티어를 변경하면 그렉텍 실제 터빈 전압 한계 공식($V_{\text{max}} = V_{\text{holder}} \times \frac{P_{\text{rotor}}}{100}$)에 맞춰 **최대 정격 병렬(Par) 및 발전량이 즉시 1초 만에 자동 계산**됩니다.

### 5. 병렬 수 자유 입력 & 마우스 휠 조절
- **직접 숫자 타이핑**: `Par` 버튼을 클릭하여 `1152` 같은 대규모 멀티블록/터빈 병렬 수를 원하는 대로 직접 키보드로 입력할 수 있습니다.
- **마우스 휠 조절**: `Par` 버튼 위에서 휠을 굴려 2배 증가/감소 또는 `Shift + 휠`로 +1/-1 정밀 조절이 가능합니다.
...
### 7. 실시간 공정 요약
- 전체 공정 가동 시 총 소모 전력(EU/t)과 최고 요구 전압을 실시간으로 표시합니다 (발전기 발전량 자동 차감).
...
./gradlew build
```
빌드된 파일은 `build/libs/gtcalcboard-1.20.1-1.0.2.jar`에 생성됩니다.
