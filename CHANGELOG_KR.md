...
- **잉여 부산물 폐기 처리 및 보이드 싱크 시스템**:
  ...
  - **요약 결산창 원클릭 보이드 및 접이식 복원**: 우측 결산창(`SummaryOverlay`)의 순 생산품 목록에서 `[🗑️]` 버튼을 클릭하여 즉시 폐기 목록으로 이동할 수 있으며, 접이식 `🗑️ 폐기된 부산물` 섹션에서 수량 확인 및 `[↩️]` 원클릭 복원을 지원합니다.
...
- **기계 가용 최대 병렬(Max Parallel) 및 오버클럭 계산 고도화**:
  - 장착된 에너지 해치 용량 및 공급 전압($P_{\max}$)에 따라 기계가 동시 가동할 수 있는 물리적 가용 병렬 수를 계산하고, 설정된 병렬 수가 하드웨어 한계를 초과할 경우 시각적 경고 제공.
...
### 신규 기능 (Added)
- **다열 및 다단계 조립 복합 모듈 자동 분할 생성 (`EmiStepRecipeDetector`, `CompoundRecipeBuilder`)**:
  - `Star Technology` 모드팩의 `Stargate Component Assembly` 및 GTCEu `Assembly Line`과 같은 다단계/다열(I~XVI 컬럼) 조립 레시피를 보드로 추가할 때, 각 단계별 입력(아이템 및 유체)을 레이어 노드(`Layer I` ~ `Layer XVI`)로 자동 분할하고 보라색 복합 모듈 프레임(`🧩 ...`)으로 묶어 배치하도록 지원.

### 버그 수정 (Fixed)
- **GTCEu 분쇄기(Macerator) 부산물 전압 티어(HV+) 요구조건 판별 오류 수정 (`GTPowerCalculator`, `NodeRateCalculator`, `BoardTooltipRenderer`)**:
  - 식물 뭉치(Plant Ball) 분쇄 등 기본 확률이 100%인 주산물 및 다중 출력 레시피에서 두 번째 슬롯(Slot 1) 이후의 모든 부산물이 ULV/LV/MV 티어에서 잘못 활성화되거나 `Requires EV+`로 왜곡되어 표시되던 문제 해결.
  - 첫 번째 슬롯(Slot 0)만 주산물로 항상 생산되고, 두 번째 슬롯(Slot 1)부터의 모든 출력물은 GTCEu 인게임 공식 사양에 맞추어 HV 티어 이상(`Requires HV+`)에서만 활성화되도록 수정.
