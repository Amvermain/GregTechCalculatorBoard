...
### 신규 기능 (Added)
- **초고전압 터빈 발전기 티어 지원 확장 (UHV ~ MAX)**:
  - GTCEu 및 Star Technology 터빈 발전기 물리 모델을 확장하여 초고전압 로터 홀더 및 다이나모 해치(UHV, UEV, UIV, UXV, OpV, MAX)에 대한 정확한 유체 유량, 최적 효율점, 기본 발전량 계산을 지원합니다.
- **보드 전용 가상 GUI Scale 독립 조절 기능**:
  - 마인크래프트 기본 GUI 배율과 완전히 독립된 보드 전용 가상 뷰포트 배율 옵션(`[⚙ 환경설정]` -> GUI Scale)을 추가하여 해상도에 맞추어 보드 UI 크기를 자유롭게 조정할 수 있습니다.
- **동적 애드온 카탈로그 및 컴팩트 리스트 뷰 (`AddonCatalogView`)**:
  - 다이얼로그 높이에 따라 카탈로그 행 수(2~5행)를 동적으로 자동 계산하며, `[▦ / ☰]` 원클릭 토글을 통해 20px 고밀도 리스트 뷰로 전환하여 10개 이상의 애드온을 스크롤 없이 비교 및 장착할 수 있습니다.
- **적응형 반응형 툴바 및 오버플로우 메뉴 (`ToolbarWidget`)**:
  - 좁은 해상도에서 타이틀을 3단계(`GT 보드`, `📟`)로 자동 축약하고, 가용 영역을 초과하는 버튼들을 `[...]` 팝업 드롭다운 메뉴로 자동 집약하도록 개선했습니다.

### 개선 및 변경 (Changed & Improved)
- **월드 접속 시 백그라운드 레시피 인덱싱 최적화**:
  - 카테고리 역량 매트릭스 빌더의 워크스테이션 목록 정렬을 O(1) Set 기반 조회로 최적화하여, 월드 및 서버 접속 시 Phase 3 백그라운드 인덱싱 구간에서 발생하던 클라이언트 프리징 및 프레임 드랍을 원천 차단했습니다.
- **Two-Pass Z-Order 렌더링 및 아이템 깊이 버퍼 격리**:
  - 캔버스 렌더링 시 노드 카드 단위의 깊이 버퍼 격리와 2패스 레이어링을 적용하여 3D 아이템 모델과 텍스트 라벨 간의 Z-clipping 간섭 및 레이어링 결함을 해소했습니다.
- **미니맵 오버레이 충돌 방지 여백 확보**:
  - 하단 툴바 우측에 16px 안전 여백을 자동 확보하여 JourneyMap 등 외부 미니맵 오버레이와의 시각적 겹침을 방지했습니다.

### 버그 수정 (Fixed)
- **창 크기 조절 시 하단 툴바 버튼 클릭 영역 오정렬 수정**:
  - 마인크래프트 창 해상도를 실시간으로 변경할 때 툴바 버튼의 클릭 판정 영역이 시각적 렌더링 위치와 어긋나거나 초과 버튼이 잘리던 문제를 해결했습니다.
- **기계 설정 모달 내 애드온 카탈로그 하단 영역 잘림 수정**:
  - 특정 디스플레이 해상도에서 기계 설정 모달을 열었을 때 애드온 카탈로그의 최하단 행이 프레임 밖으로 잘려 나가던 레이아웃 결함을 수정했습니다.
